...
class TestAIAnalyzerStandaloneAnalysis(unittest.TestCase):
    def test_standalone_only_content_should_not_be_skipped(self) -> None:
        analyzer = AIAnalyzer(
            ai_config={
                "MODEL": "openai/gpt-4o",
                "API_KEY": "test",
                        ...
                        "items": [
                            {
                                "title": "独立热点",
                                "ranks": [1],
                                "first_time": "2026-01-21 09:00:00",
                                "last_time": "2026-01-21 10:00:00",
                                "count": 2,
                            }
                        ],
                    }
        ...
        self.assertEqual(result.analyzed_news, 0)
        self.assertEqual(result.standalone_analyzed, 1)
        self.assertEqual(result.standalone_summaries, {"知乎": "独立摘要"})
        self.assertIn("独立展示区", client.messages[-1]["content"])
        self.assertIn("独立热点", client.messages[-1]["content"])
